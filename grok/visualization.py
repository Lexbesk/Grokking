...
def load_metric_data(data_dir, epochs=100, load_partial_data=True):
    ...
    for arch in archs:
        ...
        for i, t in tqdm(list(enumerate(T))):
            ...
            try:
                # print(log_dir + "/default/version_6/metrics.csv")
                with open(log_dir + "/default/version_6/metrics.csv", "r") as fh:
                    # print(log_dir + "/default/version_6/metrics.csv")
...
def add_metric_graph(
    ...
    plots = []
    for arch in metric_data:
        metric_data[arch]["T"] = torch.tensor([0])
