...
def generate_combinations(nums, k):
    return list(itertools.product(nums, repeat=k))
...
class ArithmeticDataset:
    ...
    @classmethod
    def make_data(cls, operator, operands=None, shuffle=True, seed=0, K=2) -> List[str]:
        operator, noise_level = cls._get_operator_and_noise_level(operator)
        assert operator in VALID_OPERATORS
        
        if operator == '+' and K > 2:
            print("Solving the problem of a + b + c + ... = d (mod p)")
            ...
            for comb in combinations:
                ...
                eq_list = []
                for i in range(K):
                    eq_list.append(str(comb[i]))
                    if i < K - 1:
                        eq_list.append('+')
                eq_list.append('=')
                eq_list.append(str(c))
                eq = " ".join(eq_list)
                eqs.append(eq)
            data = [EOS_TOKEN + " " + eq + " " + EOS_TOKEN for eq in eqs]
            # print(eqs)
            # print(data)
            return data
...
class ArithmeticIterator(torch.utils.data.IterableDataset):
    ...
    def calculate_batchsize(ds_size: int, batchsize_hint: int = 0) -> int:
        ...
        if batchsize_hint == -1:
            ...
            print(ds_size, min(512, math.ceil(ds_size / 2.0)))
